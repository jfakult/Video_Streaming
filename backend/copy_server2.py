...
### STREAMING CONSTANTS
STREAM_BITRATE = 9000000
STREAM_KEYFRAME_INTERVAL = 45
STREAM_WIDTH = 1920 #4624 #1920 #2312
STREAM_HEIGHT = 1080 #3472  #1080 #1736
STREAM_FPS = 30
...
def blink_led():
    while True:
        pwm_rgb.set_color(0, 80, 180, 1000)
        time.sleep(1)
...
async def start_libcamera_vid():
    ...
    try:
        process = await asyncio.create_subprocess_shell(f'sh {ROOT_PATH}/scripts/stream.sh {STREAM_BITRATE} {STREAM_KEYFRAME_INTERVAL} {STREAM_WIDTH} {STREAM_HEIGHT} {STREAM_FPS}')
    except Exception as e:
...
async def main():
    # Startup the led blinker in the background. We'll worry about status updates later
    thread = threading.Thread(target=blink_led, daemon=True)
    thread.start()
